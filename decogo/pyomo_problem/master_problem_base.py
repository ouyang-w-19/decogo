...
class MasterProblemBase:
    ...
    def __init__(self, block_model):
        ...
        def block_rule(component, k):
            ...
            for n in range(1, self.block_model.block_sizes[k - 1] + 1):
                lb = self.block_model.sub_models[k - 1].variables[
                    n - 1].lower_bound
                component.Y[n].value = lb

                component.Y[n].setlb(lb)

                ub = self.block_model.sub_models[k - 1].variables[
                    n - 1].upper_bound
                component.Y[n].setub(ub)

                if self.block_model.sub_models[k - 1].variables[n - 1].type == \
                        "Binary":
                    component.Y[n].domain = Binary
                    component.Y[n].value = 0
                elif self.block_model.sub_models[k - 1].variables[n - 1].type == \
                        "Integers":
            ...
            num_local_cuts = self.block_model.cuts.num_of_local_cuts[k - 1]
        ...
        self.model.slack_pos = \
            Var(RangeSet(self.block_model.cuts.num_of_global_cuts),
        ...
        def lin_global_con_rule(model, m):
            return sum(self.block_model.cuts.global_cuts[m].lhs[k, n] *
        ...
        if block_id is None:
            for k in range(self.block_model.num_blocks):
                for i, var_domain in enumerate(
                        self.block_model.sub_models[k].variables):
            ...
            for i, var_domain in enumerate(
                    self.block_model.sub_models[block_id].variables):
                if var_domain.type == "Integers":
        ...
        if blocks is None:
            for k in range(self.block_model.num_blocks):
                for i, var in enumerate(
                        self.block_model.sub_models[k].variables):
                    var_type = self.block_model.sub_models[k].variables[i].type
    ...
    def add_linear_local_constraint(self, block_id):
        ...
        constr = self.block_model.cuts.local_cuts[block_id][-1]
        if constr.relation == "<=":
        ...
        if rhs == float('inf') or math.isnan(rhs):
            ...
            if self.model.find_component('optimality_cut') is not None:
                ...
                self.model.optimality_cut = \
                    Constraint(
                        expr=sum(self.block_model.cuts.obj.c[k, i] *
                                 self.model.blocks[k + 1].Y[i + 1]
                                 for k in range(self.block_model.num_blocks)
                                 for i in range(self.block_model
                                                .block_sizes[k]))
    ...
    def update_var_lower_bound(self, index):
        ...
        # updated
        k, i = index
        var = self.block_model.sub_models[k].variables[i]
