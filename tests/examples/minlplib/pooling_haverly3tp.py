...
m.obj = Objective(expr= - 3*m.x4 - 9*m.x5 + 4*m.x6 - 2*m.x7 + m.x10 - 5*m.x11, sense=minimize)

m.c2 = Constraint(expr=   m.x4 + m.x5 <= 300)

m.c3 = Constraint(expr=   m.x6 + m.x7 <= 300)

m.c4 = Constraint(expr=   m.x10 + m.x11 <= 300)

m.c5 = Constraint(expr=   m.x4 + m.x5 + m.x6 + m.x7 <= 300)

m.c6 = Constraint(expr=   m.x4 + m.x6 + m.x10 <= 100)
