...
m.c17 = Constraint(expr= - m.x10 - 2*m.x14 + m.x18 - 3*m.x22 - 3*m.x26 <= 0)

m.c18 = Constraint(expr=   4*m.x10 - m.x14 + 5*m.x18 - m.x22 + 2*m.x26 <= 0)

m.c19 = Constraint(expr= - 3*m.x11 - m.x23 - 3*m.x27 <= 0)

m.c20 = Constraint(expr=   3.5*m.x11 - 1.5*m.x15 + 3*m.x19 + 0.5*m.x23 + 0.2*m.x27 <= 0)

m.c21 = Constraint(expr=   0.5*m.x11 - 0.5*m.x15 - 0.5*m.x19 - 0.5*m.x23 + 0.5*m.x27 <= 0)

m.c22 = Constraint(expr= - m.x11 + 0.5*m.x15 - 0.6*m.x19 - 0.5*m.x23 + 0.1*m.x27 <= 0)

m.c23 = Constraint(expr= - 2*m.x11 - 3*m.x15 - 4*m.x23 - 4*m.x27 <= 0)

m.c24 = Constraint(expr=   3*m.x11 - 2*m.x15 + 4*m.x19 - 2*m.x23 + m.x27 <= 0)

m.c25 = Constraint(expr= - 0.5*m.x12 + 2.5*m.x16 + 2.5*m.x20 + 1.5*m.x24 - 0.5*m.x28 <= 0)

m.c26 = Constraint(expr=   0.5*m.x12 - 4.5*m.x16 - 2.5*m.x24 - 2.8*m.x28 <= 0)

m.c27 = Constraint(expr=   0.1*m.x12 - 0.9*m.x16 - 0.9*m.x20 - 0.9*m.x24 + 0.1*m.x28 <= 0)

m.c28 = Constraint(expr= - 0.3*m.x12 + 1.2*m.x16 + 0.1*m.x20 + 0.2*m.x24 + 0.8*m.x28 <= 0)

m.c29 = Constraint(expr= - 2*m.x12 - 3*m.x16 - 4*m.x24 - 4*m.x28 <= 0)

m.c30 = Constraint(expr=   3*m.x12 - 2*m.x16 + 4*m.x20 - 2*m.x24 + m.x28 <= 0)

m.c31 = Constraint(expr= - 2*m.x13 + m.x17 + m.x21 - 2*m.x29 <= 0)

m.c32 = Constraint(expr=   2*m.x13 - 3*m.x17 + 1.5*m.x21 - m.x25 - 1.3*m.x29 <= 0)

m.c33 = Constraint(expr= - m.x17 - m.x21 - m.x25 <= 0)

m.c34 = Constraint(expr= - 1.3*m.x13 + 0.2*m.x17 - 0.9*m.x21 - 0.8*m.x25 - 0.2*m.x29 <= 0)

m.c35 = Constraint(expr= - 3*m.x13 - 4*m.x17 - m.x21 - 5*m.x25 - 5*m.x29 <= 0)

m.c36 = Constraint(expr=   3*m.x13 - 2*m.x17 + 4*m.x21 - 2*m.x25 + m.x29 <= 0)

m.c37 = Constraint(expr=   m.x2 + m.x3 + m.x4 + m.x5 == 1)

m.c38 = Constraint(expr=   m.x6 + m.x7 + m.x8 + m.x9 == 1)

m.c39 = Constraint(expr=-m.x2*m.x30 + m.x10 == 0)

m.c40 = Constraint(expr=-m.x3*m.x30 + m.x11 == 0)

m.c41 = Constraint(expr=-m.x4*m.x30 + m.x12 == 0)

m.c42 = Constraint(expr=-m.x5*m.x30 + m.x13 == 0)

m.c43 = Constraint(expr=-m.x2*m.x31 + m.x14 == 0)

m.c44 = Constraint(expr=-m.x3*m.x31 + m.x15 == 0)

m.c45 = Constraint(expr=-m.x4*m.x31 + m.x16 == 0)

m.c46 = Constraint(expr=-m.x5*m.x31 + m.x17 == 0)

m.c47 = Constraint(expr=-m.x6*m.x32 + m.x18 == 0)

m.c48 = Constraint(expr=-m.x7*m.x32 + m.x19 == 0)

m.c49 = Constraint(expr=-m.x8*m.x32 + m.x20 == 0)

m.c50 = Constraint(expr=-m.x9*m.x32 + m.x21 == 0)

m.c51 = Constraint(expr=-m.x6*m.x33 + m.x22 == 0)

m.c52 = Constraint(expr=-m.x7*m.x33 + m.x23 == 0)

m.c53 = Constraint(expr=-m.x8*m.x33 + m.x24 == 0)

m.c54 = Constraint(expr=-m.x9*m.x33 + m.x25 == 0)

m.c55 = Constraint(expr=-m.x6*m.x34 + m.x26 == 0)

m.c56 = Constraint(expr=-m.x7*m.x34 + m.x27 == 0)

m.c57 = Constraint(expr=-m.x8*m.x34 + m.x28 == 0)

m.c58 = Constraint(expr=-m.x9*m.x34 + m.x29 == 0)
