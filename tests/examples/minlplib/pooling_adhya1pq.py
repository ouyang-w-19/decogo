...
m.c17 = Constraint(expr= - 3*m.x16 - m.x28 - 3*m.x32 <= 0)

m.c18 = Constraint(expr=   3.5*m.x16 - 1.5*m.x20 + 3*m.x24 + 0.5*m.x28 + 0.2*m.x32 <= 0)

m.c19 = Constraint(expr=   0.5*m.x16 - 0.5*m.x20 - 0.5*m.x24 - 0.5*m.x28 + 0.5*m.x32 <= 0)

m.c20 = Constraint(expr= - m.x16 + 0.5*m.x20 - 0.6*m.x24 - 0.5*m.x28 + 0.1*m.x32 <= 0)

m.c21 = Constraint(expr= - 0.5*m.x17 + 2.5*m.x21 + 2.5*m.x25 + 1.5*m.x29 - 0.5*m.x33 <= 0)

m.c22 = Constraint(expr=   0.5*m.x17 - 4.5*m.x21 - 2.5*m.x29 - 2.8*m.x33 <= 0)

m.c23 = Constraint(expr=   0.1*m.x17 - 0.9*m.x21 - 0.9*m.x25 - 0.9*m.x29 + 0.1*m.x33 <= 0)

m.c24 = Constraint(expr= - 0.3*m.x17 + 1.2*m.x21 + 0.1*m.x25 + 0.2*m.x29 + 0.8*m.x33 <= 0)

m.c25 = Constraint(expr= - 2*m.x18 + m.x22 + m.x26 - 2*m.x34 <= 0)

m.c26 = Constraint(expr=   2*m.x18 - 3*m.x22 + 1.5*m.x26 - m.x30 - 1.3*m.x34 <= 0)

m.c27 = Constraint(expr= - m.x22 - m.x26 - m.x30 <= 0)

m.c28 = Constraint(expr= - 1.3*m.x18 + 0.2*m.x22 - 0.9*m.x26 - 0.8*m.x30 - 0.2*m.x34 <= 0)

m.c29 = Constraint(expr=   m.x2 + m.x3 == 1)

m.c30 = Constraint(expr=   m.x4 + m.x5 + m.x6 == 1)

m.c31 = Constraint(expr=-m.x2*m.x7 + m.x15 == 0)

m.c32 = Constraint(expr=-m.x2*m.x8 + m.x16 == 0)

m.c33 = Constraint(expr=-m.x2*m.x9 + m.x17 == 0)

m.c34 = Constraint(expr=-m.x2*m.x10 + m.x18 == 0)

m.c35 = Constraint(expr=-m.x3*m.x7 + m.x19 == 0)

m.c36 = Constraint(expr=-m.x3*m.x8 + m.x20 == 0)

m.c37 = Constraint(expr=-m.x3*m.x9 + m.x21 == 0)

m.c38 = Constraint(expr=-m.x3*m.x10 + m.x22 == 0)

m.c39 = Constraint(expr=-m.x4*m.x11 + m.x23 == 0)

m.c40 = Constraint(expr=-m.x4*m.x12 + m.x24 == 0)

m.c41 = Constraint(expr=-m.x4*m.x13 + m.x25 == 0)

m.c42 = Constraint(expr=-m.x4*m.x14 + m.x26 == 0)

m.c43 = Constraint(expr=-m.x5*m.x11 + m.x27 == 0)

m.c44 = Constraint(expr=-m.x5*m.x12 + m.x28 == 0)

m.c45 = Constraint(expr=-m.x5*m.x13 + m.x29 == 0)

m.c46 = Constraint(expr=-m.x5*m.x14 + m.x30 == 0)

m.c47 = Constraint(expr=-m.x6*m.x11 + m.x31 == 0)

m.c48 = Constraint(expr=-m.x6*m.x12 + m.x32 == 0)

m.c49 = Constraint(expr=-m.x6*m.x13 + m.x33 == 0)

m.c50 = Constraint(expr=-m.x6*m.x14 + m.x34 == 0)
