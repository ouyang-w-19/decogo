...
m.c2 = Constraint(expr= - m.x10 - m.x16 - m.x17 - m.x18 == -150)

m.c3 = Constraint(expr= - m.x11 - m.x19 - m.x20 - m.x21 == -70)

m.c4 = Constraint(expr= - m.x12 - m.x22 - m.x23 - m.x24 == -900)
...
m.c12 = Constraint(expr= - m.x56 - m.x74 - m.x77 - m.x80 == -1500)

m.c13 = Constraint(expr= - m.x57 - m.x75 - m.x78 - m.x81 == -30000)

m.c14 = Constraint(expr= - m.x58 - m.x76 - m.x79 - m.x82 == -52500)

m.c15 = Constraint(expr= - m.x59 - m.x83 - m.x86 - m.x89 == -7700)

m.c16 = Constraint(expr= - m.x60 - m.x84 - m.x87 - m.x90 == -9800)

m.c17 = Constraint(expr= - m.x61 - m.x85 - m.x88 - m.x91 == -7000)

m.c18 = Constraint(expr= - m.x62 - m.x92 - m.x95 - m.x98 == -90000)

m.c19 = Constraint(expr= - m.x63 - m.x93 - m.x96 - m.x99 == -22500)

m.c20 = Constraint(expr= - m.x64 - m.x94 - m.x97 - m.x100 == -81000)

m.c21 = Constraint(expr= - m.x74 + 1500*m.x125 == 0)

m.c22 = Constraint(expr= - m.x75 + 30000*m.x125 == 0)

m.c23 = Constraint(expr= - m.x76 + 52500*m.x125 == 0)

m.c24 = Constraint(expr= - m.x77 + 1500*m.x126 == 0)

m.c25 = Constraint(expr= - m.x78 + 30000*m.x126 == 0)

m.c26 = Constraint(expr= - m.x79 + 52500*m.x126 == 0)

m.c27 = Constraint(expr= - m.x80 + 1500*m.x127 == 0)

m.c28 = Constraint(expr= - m.x81 + 30000*m.x127 == 0)

m.c29 = Constraint(expr= - m.x82 + 52500*m.x127 == 0)

m.c30 = Constraint(expr= - m.x83 + 7700*m.x128 == 0)

m.c31 = Constraint(expr= - m.x84 + 9800*m.x128 == 0)

m.c32 = Constraint(expr= - m.x85 + 7000*m.x128 == 0)

m.c33 = Constraint(expr= - m.x86 + 7700*m.x129 == 0)

m.c34 = Constraint(expr= - m.x87 + 9800*m.x129 == 0)

m.c35 = Constraint(expr= - m.x88 + 7000*m.x129 == 0)

m.c36 = Constraint(expr= - m.x89 + 7700*m.x130 == 0)

m.c37 = Constraint(expr= - m.x90 + 9800*m.x130 == 0)

m.c38 = Constraint(expr= - m.x91 + 7000*m.x130 == 0)

m.c39 = Constraint(expr= - m.x92 + 90000*m.x131 == 0)

m.c40 = Constraint(expr= - m.x93 + 22500*m.x131 == 0)

m.c41 = Constraint(expr= - m.x94 + 81000*m.x131 == 0)

m.c42 = Constraint(expr= - m.x95 + 90000*m.x132 == 0)

m.c43 = Constraint(expr= - m.x96 + 22500*m.x132 == 0)

m.c44 = Constraint(expr= - m.x97 + 81000*m.x132 == 0)

m.c45 = Constraint(expr= - m.x98 + 90000*m.x133 == 0)

m.c46 = Constraint(expr= - m.x99 + 22500*m.x133 == 0)

m.c47 = Constraint(expr= - m.x100 + 81000*m.x133 == 0)

m.c48 = Constraint(expr= - m.x56 + 1500*m.x119 == 0)

m.c49 = Constraint(expr= - m.x57 + 30000*m.x119 == 0)

m.c50 = Constraint(expr= - m.x58 + 52500*m.x119 == 0)

m.c51 = Constraint(expr= - m.x59 + 7700*m.x120 == 0)

m.c52 = Constraint(expr= - m.x60 + 9800*m.x120 == 0)

m.c53 = Constraint(expr= - m.x61 + 7000*m.x120 == 0)

m.c54 = Constraint(expr= - m.x62 + 90000*m.x121 == 0)

m.c55 = Constraint(expr= - m.x63 + 22500*m.x121 == 0)

m.c56 = Constraint(expr= - m.x64 + 81000*m.x121 == 0)

m.c57 = Constraint(expr= - m.x16 + 150*m.x125 == 0)

m.c58 = Constraint(expr= - m.x17 + 150*m.x126 == 0)

m.c59 = Constraint(expr= - m.x18 + 150*m.x127 == 0)

m.c60 = Constraint(expr= - m.x19 + 70*m.x128 == 0)

m.c61 = Constraint(expr= - m.x20 + 70*m.x129 == 0)

m.c62 = Constraint(expr= - m.x21 + 70*m.x130 == 0)

m.c63 = Constraint(expr= - m.x22 + 900*m.x131 == 0)

m.c64 = Constraint(expr= - m.x23 + 900*m.x132 == 0)

m.c65 = Constraint(expr= - m.x24 + 900*m.x133 == 0)

m.c66 = Constraint(expr= - m.x10 + 150*m.x119 == 0)

m.c67 = Constraint(expr= - m.x11 + 70*m.x120 == 0)

m.c68 = Constraint(expr= - m.x12 + 900*m.x121 == 0)
...
m.c72 = Constraint(expr= - 145*m.x26 + m.x29 + m.x38 + m.x47 + m.x74 + m.x83 + m.x92 <= 0)

m.c73 = Constraint(expr= - 400*m.x26 + m.x30 + m.x39 + m.x48 + m.x75 + m.x84 + m.x93 <= 0)

m.c74 = Constraint(expr= - 200*m.x26 + m.x31 + m.x40 + m.x49 + m.x76 + m.x85 + m.x94 <= 0)

m.c75 = Constraint(expr= - 110*m.x27 + m.x32 + m.x41 + m.x50 + m.x77 + m.x86 + m.x95 <= 0)

m.c76 = Constraint(expr= - 90*m.x27 + m.x33 + m.x42 + m.x51 + m.x78 + m.x87 + m.x96 <= 0)

m.c77 = Constraint(expr= - 120*m.x27 + m.x34 + m.x43 + m.x52 + m.x79 + m.x88 + m.x97 <= 0)

m.c78 = Constraint(expr= - 90*m.x28 + m.x35 + m.x44 + m.x53 + m.x80 + m.x89 + m.x98 <= 0)

m.c79 = Constraint(expr= - 100*m.x28 + m.x36 + m.x45 + m.x54 + m.x81 + m.x90 + m.x99 <= 0)

m.c80 = Constraint(expr= - 300*m.x28 + m.x37 + m.x46 + m.x55 + m.x82 + m.x91 + m.x100 <= 0)

m.c81 = Constraint(expr=   0.1*m.x29 + 0.1*m.x38 + 0.1*m.x47 + 0.1*m.x74 + 0.1*m.x83 + 0.1*m.x92 - m.x101 == 0)

m.c82 = Constraint(expr=   m.x30 + m.x39 + m.x48 + m.x75 + m.x84 + m.x93 - m.x102 == 0)

m.c83 = Constraint(expr=   0.3*m.x31 + 0.3*m.x40 + 0.3*m.x49 + 0.3*m.x76 + 0.3*m.x85 + 0.3*m.x94 - m.x103 == 0)

m.c84 = Constraint(expr=   0.3*m.x32 + 0.3*m.x41 + 0.3*m.x50 + 0.3*m.x77 + 0.3*m.x86 + 0.3*m.x95 - m.x104 == 0)

m.c85 = Constraint(expr=   0.1*m.x33 + 0.1*m.x42 + 0.1*m.x51 + 0.1*m.x78 + 0.1*m.x87 + 0.1*m.x96 - m.x105 == 0)

m.c86 = Constraint(expr=   0.6*m.x34 + 0.6*m.x43 + 0.6*m.x52 + 0.6*m.x79 + 0.6*m.x88 + 0.6*m.x97 - m.x106 == 0)

m.c87 = Constraint(expr=   m.x35 + m.x44 + m.x53 + m.x80 + m.x89 + m.x98 - m.x107 == 0)

m.c88 = Constraint(expr=   0.2*m.x36 + 0.2*m.x45 + 0.2*m.x54 + 0.2*m.x81 + 0.2*m.x90 + 0.2*m.x99 - m.x108 == 0)

m.c89 = Constraint(expr=   m.x37 + m.x46 + m.x55 + m.x82 + m.x91 + m.x100 - m.x109 == 0)
...
m.c150 = Constraint(expr= - 10*m.x25 + m.x56 + m.x59 + m.x62 + m.x65 + m.x68 + m.x71 <= 0)

m.c151 = Constraint(expr= - 15*m.x25 + m.x57 + m.x60 + m.x63 + m.x66 + m.x69 + m.x72 <= 0)

m.c152 = Constraint(expr= - 20*m.x25 + m.x58 + m.x61 + m.x64 + m.x67 + m.x70 + m.x73 <= 0)
